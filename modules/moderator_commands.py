    ...
    async def handle_command(self, user: User, message: str) -> str:
        ...
        try:
            ...
            # أوامر إدارة VIP
                ...
                if not self.bot.user_manager.is_developer(user.username):
                    return f"❌ المعذرة يا {user.username}، هذا الأمر للمطورين فقط!"
                
                parts = message.split()
                if len(parts) >= 2:
                    room_id = parts[1].strip()
                    return await self.move_bot_to_room(room_id)
                else:
                    return "❌ الصيغة الصحيحة: /نقل [معرف الغرفة]"
